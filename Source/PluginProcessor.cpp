...
#endif
{
    ...
    for (int i = 0; i < 5; i++)
    {
        synth.addVoice(dynamic_cast<juce::SynthesiserVoice*>(new SynthVoice()));
    }
}
...
void WavetableSynthAudioProcessor::changeGain(float nGain)
{
    for (int i = 0; i < synth.getNumVoices(); i++)
    {
        dynamic_cast<SynthVoice*>(synth.getVoice(i))->changeGain(nGain);
    }
}

void WavetableSynthAudioProcessor::changeWaveform(int type)
{
    for (int i = 0; i < synth.getNumVoices(); i++)
    {
        dynamic_cast<SynthVoice*>(synth.getVoice(i))->changeWaveform(type);
    }
}
